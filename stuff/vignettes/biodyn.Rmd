...
1D
```{r, fig.margin=TRUE, fig.cap="Likelihood profile for r"}
res=profile(bdHat,which='r',fixed=c('b0','p'),cpue,range=seq(0.97,1.03,.005))
ggplot(res)+geom_line(aes(r,ll.u1))
```

2D
```{r, eval=FALSE, fig.margin=TRUE, fig.cap="Likelihood profile for r"}
...
```{r, fig.margin=TRUE, fig.cap="Likelihood profile by data conmponent, i.e. CPUE series"}
library(biodyn)
library(ggplot2)
library(plyr)
library(reshape)
...
prfl=profile(bd,which='r',fixed=c('b0','p'),index=Us,range=seq(0.99,1.12,.002))
prfl=melt(prfl[,c("r","ll.u1","ll.u2")],id="r")
ggplot(prfl)+geom_line(aes(r,value,group=variable,col=variable))+
...
```{r, fig.height=4, fig.margin=TRUE, fig.cap="Bootstrapped CPUE series"}
...
```{r-7,fig.fullwidth=TRUE, fig.width=4, fig.height=8,fig.cap="Densities of Stock/BMSY from different methods for estimating uncertainty."}
...
```{r-8,fig.fullwidth=TRUE, fig.width=4, fig.height=8,fig.cap="Densities of Harvest/FMSY from different methods for estimating uncertainty."}
...
## Kobe Phase Plot
```{r-9,fig.margin=TRUE,fig.width=4, fig.height=8,fig.caption="Kobe Phase Plots" }
library(kobe)
kobePhase()+
...
```{r, fig.margin=TRUE,fig.width=4, fig.height=8,fig.cap="Projection"}
...
To run annual, tri-annual, F bound and TAC bounded HCRs
```{r,fig.margin=TRUE,fig.width=4,fig.height=6}
bd=window(bd,end=29)
...
```{r,fig.margin=TRUE,fig.width=4,fig.height=6,fig.cap="Plots of projections",cache=FALSE}
bd=window(bd,end=30)
for (i in seq(29,49,1))
...
```{rhcrI,fig.fullwidth=TRUE,fig.width=6,fig.height=6}
...
```{rhcrII,fig.fullwidth = TRUE,fig.width=6,fig.height=6}
library(FLCore)
library(plyr)
library(kobe)
library(ggplot2)
