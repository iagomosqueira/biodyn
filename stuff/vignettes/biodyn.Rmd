...
## Time Series
```{r, fig.margin=TRUE, fig.width=4, fig.height=8, fig.cap="Simulated time series"}
bd=simBiodyn()
bd=window(bd,end=49)
plot(bd)
```

```{r, fig.margin=TRUE, eval=FALSE, fig.height=6, fig.width=4, echo=FALSE, fig.cap="Simulated CPUE series"}
biodyn:::plotEql(bd)
```
...
```{r, fig.margin=TRUE, fig.height=6, fig.width=4,fig.cap="Monte Carlo simuation time series with confidence intervals and a worm (single simulation)."}
...
```{r,fig.margin=TRUE,fig.width=4,fig.height=6}
setParams( bd)=cpue
params(bd)
```
...
1D
```{r1d, fig.margin=TRUE, fig.cap="Likelihood profile for r"}
load("/home/laurie/Desktop/bdHat.RData")
res=profile(bdHat,which='r',fixed=c('b0','p'),
            cpue,range=seq(0.97,1.03,.002))
ggplot(res)+geom_line(aes(r,ll.u1))
```

2D
```{r2d, eval=FALSE, fig.margin=TRUE, fig.cap="Likelihood profile for r"}
...
```{rlike, fig.margin=TRUE, fig.height=6, fig.width=4, fig.cap="Likelihood profile by data conmponent, i.e. CPUE series"}
...
```{rboot, fig.height=4, fig.margin=TRUE, fig.cap="Bootstrapped CPUE series"}
...
```{rjackknife,fig.height=4,fig.margin=TRUE, fig.cap="Plot predicted stock trend by index"}
bdJK=fit(bdHat,jackknife(cpue))
save(bdJK,file="/home/laurie/Desktop/bdJK.RData")
```

The Jack knife is a relatively quick procedure and so suitable for simulation testing
```{rjackknife2,fig.height=4,fig.margin=TRUE, fig.cap="Predicted stock trend by index"}
...
```{r,eval=FALSE,fig.margin=TRUE,fig.width=4,fig.height=6}
...
```{r-7,fig.fullwidth=TRUE, fig.width=6, fig.height=8,fig.cap="Densities of Stock/BMSY from different methods for estimating uncertainty."}
...
```{r-8,fig.fullwidth=TRUE, fig.width=6, fig.height=8,fig.cap="Densities of Harvest/FMSY from different methods for estimating uncertainty."}
...
## Kobe Phase Plot
```{r-9,fig.margin=TRUE,fig.width=4,fig.height=4,fig.caption="Kobe Phase Plots" }
library(kobe)
kobePhase()+
...
```{r, fig.margin=TRUE,fig.width=4, fig.height=6,fig.cap="Projection"}
...
```{r,fig.fullwidth=TRUE,fig.width=6,fig.height=6,fig.cap="Plots of projections"}
plot(simHCR)+
  theme(legend.position="bottom")
```


```{rhcrII,fig.margin=TRUE,fig.width=6,fig.height=6}
library(FLCore)
library(plyr)
library(kobe)
library(ggplot2)
...
kobePhase(subset(kb,year%in%i))+    
  ...
  geom_point(aes(stock,harvest,fill=.id,group=.id),
             data=subset(pt.,year==i),shape=21,col="black",size=5)+    
...
```{rhcr-ts,fig.fullwidth=TRUE,fig.width=6,fig.height=6}
p.=plot(simHCR)+    
  theme(legend.position="bottom")+    
  scale_colour_manual(values=c("cyan4","grey10","red","blue"),
                      labels=c("HCR","10% Constraint on Effort"))+    
  guides(col=guide_legend(title=NULL))+    
  scale_x_continuous(limits=c(27,50),breaks=c(30,40,50),labels=c(1,11,21))+    
  scale_y_continuous(breaks=NULL)
...
```{rhcrI,eval=FALSE,fig.margin=TRUE,fig.width=6,fig.height=6}
...
print(kobePhase(subset(kb,year%in%i))+    
        ...
        geom_point(aes(stock,harvest,group=.id,fill=.id),  
                   col="black", data=subset(pt.,year==i),shape=21,size=5)+          
