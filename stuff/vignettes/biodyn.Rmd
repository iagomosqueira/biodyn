...
```{rinitPurl,eval=FALSE,echo=FALSE}
library(knitr)
...
```{rclassCoerce,eval=FALSE}
data(ple4)
bd =as(ple4,"biodyn")
```

or aspic
```{rclassCoerce2,eval=FALSE}
library(aspic)
...
## Time Series
```{rplot, fig.margin=TRUE, fig.width=4, fig.height=8, fig.cap="Simulated time series"}
bd=simBiodyn()
bd=window(bd,end=49)
plot(bd)
```

```{rplotEql, fig.margin=TRUE, eval=FALSE, fig.height=6, fig.width=4, echo=FALSE, fig.cap="Simulated CPUE series"}
biodyn:::plotEql(bd)
```

## Production Function
```{rplotPrd, fig.margin=TRUE, fig.cap="Simulated CPUE series"}
library(reshape)
x=simBiodyn()
plotPrd(x)+
...
```{rplotMSE, fig.margin=TRUE, eval=FALSE, fig.cap="Simulated CPUE series"}
plotMSE()
```
\newpage
...
```{rplotMC, fig.margin=TRUE, fig.height=6, fig.width=4,fig.cap="Monte Carlo simuation time series with confidence intervals and a worm (single simulation)."}
...
```{rfit, echo=TRUE, fig.margin=TRUE, fig.height=4, fig.cap="Simulated stock"}
bd=simBiodyn()
```

A CPUE series is also needed for fitting and can be simulated that by taking the mid year biomass and adding error.

```{rfitU, fig.margin=TRUE, fig.cap="Simulated CPUE series"}
...
```{rfitParams,fig.margin=TRUE,fig.width=4,fig.height=6}
setParams( bd)=cpue
params(bd)
```
...
```{rfitControl,fig.margin=TRUE,fig.width=4,fig.height=6}
...
```{rfitRun, eval=TRUE, fig.margin=TRUE, fig.height=6,fig.cap="A comparison of the true and fitted time series"}
...
```{rfitCheck,fig.margin=TRUE,fig.width=4,fig.height=6}
params(bdHat)
...
```{rdiagQQ, fig.width=4,fig.height=4,fig.margin=TRUE, fig.cap="Quantile-quantile plot to compare residual distribution with the normal distribution."}
rsdl=bdHat@diags
ggplot(rsdl)                                           +
...
```{rdiagHat, fig.margin=TRUE, fig.height=4, figwidth=4, fig.cap="Observed CPUE verses fitted, blue line is a linear resgression fitted to points, black the y=x line."}
...
```{rdiagYr,fig.height=3, fig.margin=TRUE, fig.cap="Residuals by year, with lowess smoother"}
...
```{rdiagVar,fig.height=3, fig.margin=TRUE, fig.cap="Plot of residuals against fitted value, to check variance relationship."}
...
```{rdiagAR, fig.width=4,fig.width=4,fig.margin=TRUE, fig.cap="Plot of autocorrelation, i.e. $residual_{t+1}$ verses $residual_{t}$."}
ggplot(rsdl)                                              +
...
1D
```{rprfl, fig.margin=TRUE, fig.cap="Likelihood profile for r"}
...
2D
```{rprfl2, eval=FALSE, fig.margin=TRUE, fig.cap="Likelihood profile for r"}
...
```{rprflLike, fig.margin=TRUE, fig.height=6, fig.width=4, fig.cap="Likelihood profile by data conmponent, i.e. CPUE series"}
...
Profile Slot
```{rprflADMB,echo=FALSE,eval=FALSE}
...
```{runcertainty, fig.margin=TRUE}
bd   =window(simBiodyn(),end=39)
cpue=(stock(bd)[,-dims(bd)$year]+
      stock(bd)[,-1])/2
cpue=rlnorm(1,log(cpue),.2)
bdHat=bd
...
```{runcertaintyCov,fig.height=6, fig.margin=TRUE}
vcov(bdHat)
```
...
```{runcertaintyBoot, fig.height=4, fig.margin=TRUE, fig.cap="Bootstrapped CPUE series"}
...
```{runcertaintyJackknife,fig.height=4,fig.margin=TRUE, fig.cap="Plot predicted stock trend by index"}
...
```{runcertaintyMCMC2,fig.height=4,fig.margin=TRUE}
...
```{rref3,eval=FALSE,fig.margin=TRUE,fig.width=4,fig.height=6,fig.cap=""}
...
```{rrefBmsy,fig.fullwidth=TRUE, fig.width=4, fig.height=6,fig.cap="Densities of Stock/BMSY from different methods for estimating uncertainty."}
...
rnorm(500,sims[["Best Fit"]]@mng["bnow","hat"],sims[["Best Fit"]]@mng["bnow","sd"])
stock(sims[["CPUE"]])[,39]
stock(sims[["MCMC"]])[,39]
randJack(500,stock(sims[["Best Fit"]])[,39],stock(sims[["Jack Knife"]])[,39])


df=kobe(sims[-c(1:2)])
ggplot(subset(df,year==39))+ 
  geom_density(aes(x=stock, y=..count..), position = "stack",fill="red")+
  facet_wrap(~.id,scale="free_y",ncol=1)+
  geom_vline(aes(xintercept=c(stock(sims[["Best Fit"]])[,"38"]%/%bmsy(sims[["Best Fit"]]))))

  #scale_x_continuous(limits=c(0.5,1.5))
```

## Marginal Density for Harvest/FMSY
```{rrefFMSY,fig.fullwidth=TRUE, fig.width=6, fig.height=8,fig.cap="Densities of Harvest/FMSY from different methods for estimating uncertainty."}
ggplot(subset(df,year==39))+ 
  geom_density(aes(x=harvest, y=..count..), position = "stack",fill="red")+
  facet_wrap(~.id,scale="free_y",ncol=1)+
  scale_x_continuous(limits=c(0,2.0))+
  geom_vline(aes(xintercept=c(harvest(sims[["Best Fit"]])[,"38"]%/%fmsy(sims[["Best Fit"]]))))

```

## Kobe Phase Plot
```{rkobe,fig.margin=TRUE,fig.width=4,fig.height=4,fig.caption="Kobe Phase Plots" }
library(kobe)
kobePhase()+
...
# Simulation
```{runcertaintyJackknife2,fig.height=4,fig.margin=TRUE,eval=FALSE}
...
```{runcertaintyCov2,fig.height=6, fig.margin=TRUE,eval=FALSE}
sims[["Vcov"]]=bdHat
...
```{rfdwd, fig.margin=TRUE,fig.width=4, fig.height=6,fig.cap="Projection"}
...
```{rhcrPlot,fig.fullwidth=TRUE,fig.width=6,fig.height=6,fig.cap="Plots of projections"}
plot(simHCR)+
  theme(legend.position="bottom")
...
```{rMC,fig.margin=TRUE,fig.width=6,fig.height=6}
...
```{rMCkobe,fig.margin=TRUE,fig.width=6,fig.height=6}
